namespace EphemereGames.Commander.Simulation
{
    ...
    class CelestialBodyNearHitAnimation
    {
        ...
        public CelestialBodyNearHitAnimation(Simulator simulator, List<Enemy> enemies, Path path)
        {
            ...
            {
                { Size.Small, CreateNearHitMaskImage(GetNearHitMaskName(Size.Small)) },
                { Size.Normal, CreateNearHitMaskImage(GetNearHitMaskName(Size.Normal)) },
                { Size.Big, CreateNearHitMaskImage(GetNearHitMaskName(Size.Big)) },
            };
        ...
        private string GetNearHitMaskName(Size size)
        {
            return "CBMask1" + ((size == Size.Small) ? 1 : (size == Size.Normal) ? 2 : 3).ToString();
        }


        private Image CreateNearHitMaskImage(string maskName)
        {
