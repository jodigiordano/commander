...
namespace Bnoerj.AI.Steering
{
	...
	public abstract class SteerLibrary : AbstractVehicle
	{
        ...
        // Arrival behavior
        public float DistanceForSmoothArrival { get; set; }
        public float NumberOfStepsForSmoothArrival { get; set; }

        public Vector3 SteerForWander(float dt, float multi)
		{
			// random walk WanderSide and WanderUp between -1 and +1
            float speed = multi * dt;
        ...
        {
            Vector3 distanceToTravel = target - this.Position;
            Vector3 steering = distanceToTravel - this.Velocity;
            float distanceLength = distanceToTravel.Length();

            if (distanceLength < this.DistanceForSmoothArrival)
                //steering *= (distanceLength / this.DistanceForSmoothArrival);
